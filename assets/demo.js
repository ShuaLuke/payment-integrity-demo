...
(function () {
  ...
  var SHORT_STEPS = [
    ...
    { t: "Scored in real time, before it pays", chip: "2 · Detect", n: "That Sonoran claim lands here, at the top of the pre-payment queue: risk 93, and the model recommends Deny. Rules and models score every claim as it's submitted, before any money goes out. Click Deny and watch Payment prevented go from $0 to $17,280. The improper payment never leaves the agency.", a: function () { closeCopilot(); window.APP.setRole("analyst"); window.APP.setMode("prepay"); window.APP.nav("queue"); setTimeout(function () { var row = [].slice.call(document.querySelectorAll("tr")).filter(function (r) { return /#?20721/.test(r.textContent); })[0]; if (row) { row.style.outline = "2px solid #17b3a6"; row.style.outlineOffset = "-2px"; } }, 80); } },
