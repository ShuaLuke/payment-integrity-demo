...
(function () {
  ...
  var FULL_STEPS = [
    ...
    // ===== Close =====
    { t: "Prepay — prevent, don't chase", pws: "—", n: "Flip the toggle to Prepay and the same models score claims before payment. The analyst decides Pay · Hold · Deny with a coded reason — a chain readmission is denied up front, so the improper payment never leaves the VA. Retrospective recovers; prepay prevents.", a: function () { window.APP.setRole("analyst"); window.APP.setMode("prepay"); window.APP.nav("queue"); } },
  ...
  // Short tour — four capabilities for an executive briefing (6 steps, ~5–7 min):
  ...
  var SHORT_STEPS = [
    { t: "Ingesting the data", chip: "1 · Ingest", n: "It starts with intake. Insights › EDI shows claims arriving in standard formats (837 claims, 835 remittances, 834 enrollment, 270/271 eligibility) with acceptance stats, top rejection reasons, and the pipeline from ingest through validation and enrichment to the models. Bad transactions are rejected at the door, and every claim lands as one normalized record that maps to both X12 and HL7 FHIR. Detection runs on clean, standard data.", a: function () { retro(); closeCopilot(); window.APP.nav("edi"); } },

    { t: "Scored in real time, before it pays", chip: "2 · Detect", n: "In Prepay mode, the rules and models score each claim as it's submitted, before any money goes out. The queue is ranked by risk, and each claim gets a decision (Pay, Hold or Deny) with a coded reason. A chain readmission is denied up front, so the improper payment never leaves the agency. Prevent first, recover second.", a: function () { closeCopilot(); window.APP.setRole("analyst"); window.APP.setMode("prepay"); window.APP.nav("queue"); } },
    { t: "Every flag explains itself", chip: "2 · Detect", n: "Each flag carries a risk score, a confidence and a plain-language reason, tied down to the exact claim line and the rule or model that fired. On this residential claim, the per-diem line is flagged because the stay runs past medical necessity and the facility billed more patient-days than its staffed beds can physically hold. A finding an investigator can act on, not a black-box score.", a: function () { retro(); closeCopilot(); window.APP.openAllegation("20544"); tab("evidence"); } },

    { t: "AI that does the legwork", chip: "3 · Case mgmt", n: "Every flag becomes a lead with a tracked lifecycle, from Flagged through Decision to Case, with a full audit trail. The Investigative Assistant is agentic: three specialized agents read the case (entity and network, billing and coding, policy and authorities), return grounded findings with their sources, and draft the correspondence. Hours of research compressed into minutes.", a: function () { retro(); window.APP.openAllegation("20481"); if (window.COPILOT) window.COPILOT.open("agents"); } },
    { t: "Human decision, supervisor control", chip: "3 · Case mgmt", n: "The analyst records the outcome (Dismiss, or Confirm / Escalate) with a coded reason and an AI-drafted justification, and every decision feeds back to retrain the models. It then routes to a supervisor: only the supervisor can open the case, release a recovery or refer to OIG or law enforcement. The human decides, the AI supports, and separation of duties is built in.", a: function () { retro(); closeCopilot(); var me = window.APP.ROLES[window.APP.state.role].name; window.APP.assignCase("20318", me); window.APP.startLeadReview("20318"); window.APP.openAllegation("20318"); tab("decision"); var seg = document.querySelector('.seg[data-d="c"]'); if (seg) seg.click(); } },

    { t: "Uncovering the network", chip: "4 · Networks", n: "Fraud is rarely one provider. The Network view links providers, billing entities, owners and beneficiaries, and separates a coordinated ring from an isolated anomaly. Here Rio Grande Surgical shares a billing tax ID with Alamo: one entity behind two fronts. Every entity is also screened against exclusion lists and outside intelligence (Pacific Sands, in the same chain, is on the OIG exclusion list), and Insights › Businesses follows the money up to the holding company.", a: function () { retro(); closeCopilot(); window.APP.openAllegation("20517"); tab("network"); } }
  ];
  ...
  var DEMO = {
    ...
    render: function () {
      ...
      q("#demo-ribbon").innerHTML =
        ...
        '</div></div>' +
        '<div style="font-size:12.5px;color:#cfe0f0;margin-top:5px;line-height:1.45;height:56px;overflow-y:auto"><span style="font-weight:500;color:#fff">' + titlePrefix + s.t + '.</span> ' + s.n + '</div>' +
        '</div>';
