...
(function () {
  ...
  // Sequenced to mirror the RFI talk-track: Orientation (1.1.i) → Core healthcare
  // entities (1.1.ii) → Healthcare standards on the claim (1.1.iii) → Lead-to-Case
  // workflow (1.1.iv) → the Rulebook (1.2.iii) → Prepay + the human-in-the-loop close.
  var STEPS = [
    // ---- Act 1 · Orientation (1.1.i) ----
    ...
    // ---- Act 2 · Core healthcare entities (1.1.ii) ----
    ...
    // ---- Act 3 · Healthcare standards on the claim (1.1.iii) ----
    ...
    { t: "Pricing — show the math", n: "Pricing compares the submitted charge to the CMS-allowed amount per line — but flip to the Calculation view and it opens the pricer itself: the RVU × GPCI × conversion-factor build-up on a professional line, the MS-DRG grouper and per-diem logic on institutional claims, APC status indicators on outpatient, and the ordered pricer log. The allowed amount is fully traceable, and each pricing rule carries its version history.", a: function () { retro(); window.APP.openAllegation("20481"); tab("pricing"); setTimeout(function () { var seg = document.querySelector('.pr-seg[data-pr="calculation"]'); if (seg) seg.click(); }, 40); } },
    ...
    // ---- Act 4 · Lead → Case workflow (1.1.iv) ----
    ...
    // ---- Act 6 · Prepay & the human-in-the-loop close ----
