...
(function () {
  ...
  var SHORT_STEPS = [
    { t: "Ingesting the data", chip: "1 · Ingest", n: "This is live intake. Data sources shows every feed we pull and from where: claims, remittance and eligibility from the transaction streams, plus provider registries, the OIG and SAM exclusion lists, death records, licensure, corporate filings and adverse media. Incoming records scroll in as they arrive. Watch for Sonoran Recovery Center: a $17,280 claim scored 93 and held. Scroll down to see the last 24 hours moving through the pipeline, from ingest to remediation.", a: function () { retro(); closeCopilot(); window.APP.nav("edi"); } },

    { t: "Scored in real time, before it pays", chip: "2 · Detect", n: "That Sonoran claim lands here, at the top of the pre-payment queue: risk 93, and the model recommends Deny. Rules and models score every claim as it's submitted, before any money goes out. Hover over the model's Deny to see why; click it and the Investigative Assistant explains it in full. Then click Deny and watch Payment prevented go from $0 to $17,280. The improper payment never leaves the agency.", a: function () { closeCopilot(); window.APP.setRole("analyst"); window.APP.setMode("prepay"); window.APP.nav("queue"); setTimeout(function () { var row = [].slice.call(document.querySelectorAll("tr")).filter(function (r) { return /#?20721/.test(r.textContent); })[0]; if (row) { row.style.outline = "2px solid #17b3a6"; row.style.outlineOffset = "-2px"; } }, 80); } },
    { t: "Every flag explains itself", chip: "2 · Detect", n: "And Sonoran has done this before. This is one of its claims that was already paid. The flag is explained down to the claim line: the per-diem stay runs past medical necessity, and the facility billed more patient-days than its staffed beds can physically hold. We stop the next one before it pays and recover this one.", a: function () { retro(); closeCopilot(); window.APP.openAllegation("20544"); tab("evidence"); } },

    { t: "AI that does the legwork", chip: "3 · Case mgmt", n: "The analyst works the lead, and the Investigative Assistant's three agents do the research: entity and network, billing and coding, policy and authorities. Each returns findings with sources. The investigative agent has already found that Sonoran isn't alone: it's one of four facilities under Meridian Behavioral Holdings, cycling the same veterans across Arizona, California and Nevada.", a: function () { retro(); window.APP.openAllegation("20544"); if (window.COPILOT) window.COPILOT.open("agents"); } },
    { t: "A documented decision", chip: "3 · Case mgmt", n: "Here's a sister facility, Pacific Sands, which is also on the OIG exclusion list. The analyst confirms it's improper, records a coded reason with an AI-drafted justification, and PIVOT suggests where it belongs: on the Sonoran case, because the two facilities share a business registration. Every decision feeds back to retrain the models.", a: function () { retro(); closeCopilot(); var me = window.APP.ROLES[window.APP.state.role].name; window.APP.assignCase("20538", me); window.APP.startLeadReview("20538"); window.APP.openAllegation("20538"); tab("decision"); var seg = document.querySelector('.seg[data-d="c"]'); if (seg) seg.click(); } },

    { t: "Uncovering the network", chip: "4 · Networks", n: "Now the full picture. Meridian Behavioral Holdings runs four facilities in three states under one officer, each billing under a separate tax ID to hide the common ownership, and the same veterans are cycled between them for back-to-back short stays. One $17,280 claim stopped at the door led us to a four-state ring.", a: function () { retro(); closeCopilot(); window.APP.nav("network"); setTimeout(function () { var b = document.querySelector('.nscn[data-scn="chain"]'); if (b) b.click(); }, 60); } }
  ];
