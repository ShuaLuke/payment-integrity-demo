...
(function () {
  ...
  var STEPS = [
    { t: "The work queue", n: "The analyst's prioritized queue — 24 post-payment claims flagged by the rules and pattern engines, sorted by risk. Drag “Escalate ≥” to raise the threshold.", a: function () { window.APP.setRole("analyst"); window.APP.nav("queue"); } },
    ...
    { t: "Supervisor review", n: "Switch to Supervisor (Karen Boyd). Confirmations wait here; Approve releases the recovery, or Return sends it back with a note. Only approval moves the money.", a: function () { window.APP.setRole("supervisor"); window.APP.nav("supervisor"); } },
    ...
    { t: "Explore the rest", n: "Provider 360, the utilization heatmap, the rules library, investigations, and the immutable audit trail are all in the top nav. That's the end-to-end payment-integrity flow.", a: function () { window.APP.nav("queue"); } }
  ];
  ...
  var DEMO = {
    ...
    pill: function () {
      ...
      if (!p) {
        ...
        p.id = "demo-pill";
        p.style.cssText = "position:fixed;bottom:18px;right:18px;z-index:200;background:#10243b;color:#fff;border:0.5px solid rgba(255,255,255,0.2);border-radius:22px;padding:8px 14px;font-size:12.5px;font-weight:500;cursor:pointer;display:flex;align-items:center;gap:7px;box-shadow:0 2px 12px rgba(0,0,0,0.2)";
