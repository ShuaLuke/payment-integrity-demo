...
(function () {
  ...
  function build() {
    ...
    ov.innerHTML =
      ...
      '<div style="display:flex;align-items:center;gap:8px;padding:11px 14px;border-bottom:0.5px solid var(--border2)"><i class="ti ti-search" style="color:var(--text2)"></i><input id="gs-input" placeholder="Search flagged claims, providers, NPIs…" style="flex:1;border:none;outline:none;font-size:14px;background:transparent;color:var(--ink);font-family:inherit"><span style="font-size:10px;color:var(--text3);border:0.5px solid var(--border);border-radius:4px;padding:1px 5px">esc</span></div>' +
  ...
  function render(q) {
    ...
    document.getElementById("gs-results").innerHTML = list.length ? list.map(function (r) {
      var icon = r.type === "claim" ? "flag" : "building-hospital";
      return '<div class="gs-row" data-type="' + r.type + '" data-id="' + r.id + '" style="display:flex;align-items:center;gap:10px;padding:9px 14px;cursor:pointer;border-top:0.5px solid var(--border2)"><i class="ti ti-' + icon + '" style="color:var(--accent-d)"></i>' +
        ...
        (r.risk ? window.UI.riskChip(r.risk) : '<span class="tag">Provider</span>') + '</div>';
    }).join("") : (q.trim() ? '<div class="muted" style="padding:14px;font-size:12.5px">No matches.</div>' : '<div class="muted" style="padding:14px;font-size:12px">Type a claim #, provider name, or NPI.</div>');
    document.querySelectorAll(".gs-row").forEach(function (row) {
      row.addEventListener("click", function () {
        var t = row.getAttribute("data-type"), id = row.getAttribute("data-id");
        close();
        if (t === "claim") window.APP.openAllegation(id); else window.APP.openProvider(id);
      });
    });
  }
